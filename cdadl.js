    ...
    try{
        ...
        if (result.uri.indexOf("?wers") > -1) {
            ...
            $("[id^=mediaplayer]").first().each(function(a) {
                ...
                var movieURL = findBetween(result.body, ss, "\\';");

                downloadMovie(movieURL);
            });

            // old method didn't work so trye with new one. its obfuscated JS so use http://deobfuscatejavascript.com/#
            ...
            var code = "eval(function(p,a,c,k,e,d)" + $("script").text().split("eval(function(p,a,c,k,e,d)")[1].split("\n")[0];
//            console.log(code);
//            console.log("\n\n");
